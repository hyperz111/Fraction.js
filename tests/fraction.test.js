...
var tests = [
  ...
  },
  {
    ...
    param: 0,
    expect:
      '3.21()',
  },
  {
    label: 'ceil(0.4)',
    set: 0.4,
  ...
  },
  {
    ...
    param: 7,
    expect:
      '0.(058527663465935070873342478280749885688157293095564700502972107910379515317786922725194330132601737540009144947416552354823959762231367169638774577046181984453589391860996799268404206675811614083219021490626428898033836305441243712848651120256)',
  },
  {
    label: '-0.6^4',
    set: -0.6,
...
describe('JSON', function (it) {
  it('Should be possible to stringify the object', function () {
    if (typeof new Fraction(1).n !== 'number') {
      return;
    }
    assert.equal(
      '{"s":1,"n":14623,"d":330}',
      JSON.stringify(new Fraction('44.3(12)')),
    );
    assert.equal(
      '{"s":-1,"n":2,"d":1}',
      JSON.stringify(new Fraction(-1 / 2).inverse()),
    );
  });
});
